...
def calculate_damage():
    try:
        ...
        basic_coeff, e_skill_coeff, q_skill_coeff,b_skill_coeff = skill_coefficients.get(character, (0, 0, 0,0))
        ...
        if character == "웰트":
            ...
            e_skill_damage = attack * e_skill_coeff
            q_skill_damage = attack * q_skill_coeff
            b_skill_damage = 4083 * b_skill_coeff * 3
            ...
            total_damage = f"기본 공격 데미지: {basic_damage:.2f}\n" \
                           f"E 스킬 데미지: {e_skill_damage:.2f}\n" \
                           f"Q 스킬 데미지: {q_skill_damage:.2f}\n" \
                           f"격파 데미지: {b_skill_damage: .2f}"

        # 결과 표시
        result_label.config(text=f"총 데미지(격파데미지 미포함): {total_damage:.2f}" if isinstance(total_damage, float) else total_damage)
...
skill_coefficients = {
    #개척자
    "개척자(파멸)": (1.1, 1.37, 4.8,2),  
    "개척자(보존)": (1.20, 1.40, 2.20,2),  
    "개척자(화합)": (1.10, 1.30, 1.80,0.5),  
    #은하열차
    "Mar.7th": (1.1, 0, 1.62,0.5),
    "단항": (1.1, 2.86, 4.32,1.5),
    "히메코": (1.1, 2.2, 2.48,2),
    "웰트": (1.1, 0.79, 1.62,0.5),
    #스텔라론
    "은랑": (1.1, 2.15, 4.1,0.5),
    "카프카":(1.1,1.76,0.88,1),
    "반디":(2.2,2.2,0,2),
    ...
    "아를란":(1.1,2.64,3.45,1),
    "아스타":(1.1,0.55,0,2),
    "헤르타":(1.1,1.1,2.16,0.5),
    #벨로보그
    "게파드":(1.1,2.2,0,0.5),
    "클라라":(1.1,1.32,3.48,2),
    "서벌":(1.1,1.54,1.94,1),
    "페라":(1.1,2.31,1.08,0.5),
    "삼포":(1.1,0.61,1.72,1.5),
    "후크":(1.1,3.08,4.32,2),
    "제레":(1.1,2.42,4.59,0.5),
    "루카":(1.54,1.32,3.56,2),
    #스타피스 컴퍼니
    "토파즈":(1.1,3.3,3.3,2),
    "어벤츄린":(1.1,0.27,2.92,0.5),
    "제이드":(0.99,1.59,2.64,0.5),
    #페나코니
    "미샤":(1.1,2.2,0,0.5),
    "갤러거":(1.1,2.75,1.65,2),
    #선주 나부
    "연경":(1.1,2.42,3.78,0.5),
    "청작":(1.1,2.64,2.16,0.5),
    "소상":(1.1,2.31,3.45,2),
    "신경원":(1.1,1.1,2.16,1),
    "단항·음월":(5.5,0,3.24,0.5),
    "경류":(1.1,2.75,3.24,0.5),
    "계네빈":(1.1,1.32,1.296,2),
    "설의":(1.1,1.54,2.7,0.5),
    "운리":(1.1,1.32,2.37,2),
    "비소":(1.1,2.2,7.6,1.5),
    "맥택":(1.1,1.65,2.92,1),
    #은하
    "아젠티":(1.1,1.32,3.02,2),
    "Dr.레이시오":(1.3,1.87,2.88,0.5),
    "블랙스완":(0.66,0.99,1.27,1.5),
    "아케론":(1.1,1.76,4.02,1),
    "부트힐":(2.42,0,4.32,2),
    "라파":(1.1,1.32,1,0.5)
}
